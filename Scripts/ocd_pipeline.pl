...
`ls -d /home/bmi/Documents/MRS/projects/fMRS_dataset_20201007/ocd748b/*m_8m*.ima | sort -V > ima_list.in`;
...
while ($ima_file = <IMA_LIST>){
        chomp($ima_file);
	my($filename, $dirs, $suffix) = fileparse($ima_file);
	#Get water unsupressed file
	@split_filename = split(/_\dm/, $filename);

	@metabolite_number = split(/_m_/, $filename);
	$water_number = substr($metabolite_number[1], 0, 1) + 1;

	$water_filename =  "$split_filename[0]_${water_number}w1.ima";
	
	if (-e "${dirs}${water_filename}") {
	        `./lcm_create_control $ima_file ${dirs}${water_filename}`;
	}else{
